...
class Colour:
    ...
    color = {
        ...
        'mode': {   # 显示模式
            ...
            'note': 3 if utils.get_current_shell_type() not in ('powershell') else 2,
            'underline': 4,  # 使用下划线
            'blink': 5,  # 闪烁
            'invert': 7,  # 反白显示
            'hide': 8,  # 不可见
    ...
    def __init__(self):
        shell_type = utils.get_current_shell_type()
        self.enable = False if shell_type in ('cmd') else True  # 指定着色是否可用
    ...
    def colorize(self, string: str, mode:Union[str, List[str]]='', fore='', back='') -> str:
        r'''对字符串着色
