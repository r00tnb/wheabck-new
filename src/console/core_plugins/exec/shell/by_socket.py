...
class SocketShell(BaseShell):
    r'''使用socket生成一个交互式shell
    '''
    def __init__(self, session:Session, shell: str):
        super().__init__(session)
        self.shell = shell

        self.socket_port = 0 # 用于进行命令交互的socket端口 
    ...
    def close(self):
        self._lock.acquire()
        if self.running:
            self.session.evalfile('interact_socket/close', dict(pwd=self.session.server_info.pwd))
            self.running = False
